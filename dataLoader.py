...
class train_loader(object):
    def __init__(self, trialFileName, audioPath, visualPath, batchSize, **kwargs):
        ...
        #change
        exist_videos = os.listdir(audioPath)
        mixLst = [item for item in mixLst if item[:11] in exist_videos]
...
class val_loader(object):
    def __init__(self, trialFileName, audioPath, visualPath, **kwargs):
        ...
        #change
        exist_videos = os.listdir(audioPath)
        print(exist_videos,len(exist_videos),'audioPath--------',audioPath)
        self.miniBatch = [item for item in self.miniBatch if item[:11] in exist_videos]
