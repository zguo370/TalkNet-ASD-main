import os, subprocess, glob, pandas, tqdm, cv2, numpy, ffmpeg
...
def extract_audio(args):
    ...
    for dataType in ['trainval']:
        ...
        for videoPath in tqdm.tqdm(videos):
            ...
            # print(audioPath,videoPath) /Users/zilinguo/opt/anaconda3/envs/TalkNet/lib/python3.7/site-packages
            # cmd = ("ffmpeg -y -i %s -async 1 -ac 1 -vn -acodec pcm_s16le\
            #  -ar 16000 -threads 8 %s -loglevel panic" % (videoPath, audioPath))
            cmd = ("/Users/zilinguo/opt/anaconda3/bin/ffmpeg -y -i %s -async 1 -ac 1 -vn -acodec pcm_s16le\
                         -ar 16000 -threads 8 %s -loglevel panic" % (videoPath, audioPath))
            print('33333333344444f')
            subprocess.call(cmd, shell=True, stdout=None)
...
def extract_audio_clips(args):
    ...
    for dataType in ['train', 'val', 'test']:
        ...
        #beacuse we only download partial video
        exist_videos=os.listdir(audioDir)
        exist_videos=[a.split('.')[0] for a in exist_videos]
        ...
        for entity in tqdm.tqdm(entityList, total = len(entityList)):
            ...
            #change part
            if videoKey not in exist_videos:
                continue
...
def extract_video_clips(args):
    ...
    # dic = {'train':'trainval', 'val':'trainval', 'test':'test'}
    dic = {'train': 'trainval', 'val': 'trainval'}
